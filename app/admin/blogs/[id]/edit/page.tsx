            ...
            <div className="mx-auto max-w-3xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border border-border/50 bg-card/50 p-6">
                    ...
                    <div className="grid gap-2">
                        ...
                        <Input id="slug" value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} className="font-mono text-sm" />
                    </div>
                    ...
                    <div className="grid gap-2">
                        ...
                        {coverImage ? (
                            <div className="relative overflow-hidden rounded-xl border border-border/50">
                                ...
                                <Button type="button" size="sm" variant="secondary" className="absolute bottom-3 right-3 gap-2" onClick={() => fileRef.current?.click()}>
                                    <Upload className="h-3.5 w-3.5" /> Change
                                </Button>
                            </div>
                        ) : (
                            <button type="button" onClick={() => fileRef.current?.click()} className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-border/50 bg-muted/20 py-10 text-muted-foreground transition-colors hover:border-primary/30">
                                {uploading ? <Loader2 className="mb-2 h-8 w-8 animate-spin text-primary" /> : <ImageIcon className="mb-2 h-8 w-8" />}
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Label htmlFor="published" className="cursor-pointer">{form.is_published ? 'Published' : 'Draft'}</Label>
                    </div>
